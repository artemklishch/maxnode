...
class Feed extends Component {
  ...
  loadPosts = (direction) => {
    ...
    const graphqlQuery = {
      ...
      // другой формат описания квери запроса
      query: `
  ...
  statusUpdateHandler = (event) => {
    ...
    const graphqlQuery = {
      // query: `
      //   mutation{ updateStatus(status: "${this.state.status}"){ status } }
      // `,
      query: `
    ...
    fetch("http://localhost:8080/post-image", {
      ...
      .then((fileResData) => {
        ...
        let graphqlQuery = {
          ...
          // `,
          query: `
            mutation CreateNewUser($title: String!, $content: String!, $imageUrl: String!){
              createPost(postInput: {
                title: $title,
                content: $content,
                imageUrl: $imageUrl
      ...
      .then((resData) => {
        ...
        let resFieldData = this.state.editPost ? "updatePost" : "createPost";
        const post = {
